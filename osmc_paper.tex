...
\begin{figure}[tbp]
	\centering
	% \vspace{-10ex}
	\includegraphics[width=0.7\textwidth]{impedance_curves_drivers}
...
\begin{figure}[tbp]
	\centering
	\vspace{-10ex}
	\includegraphics[width=0.7\textwidth]{SPL_curves_raw}
...
\begin{figure}[tb]
	\centering
	\includegraphics[width=0.6\textwidth]{osmc_bass_tuning}
...
Bits and pieces			&	approx.~50 	& Screws, wires, connectors, etc. \\
\midrule 
Total				&	approx.~1500 \\
\bottomrule
\end{tabular}
...
\begin{figure}[tb]
	\centering
	\includegraphics[width=0.7\textwidth]{osmc_impedance_curve}
...
\begin{figure}[tb]
	\centering
	\includegraphics[width=0.7\textwidth]{osmc_filter_transfer}
...
\subsection{Acoustic}
\figr{osmc_step_response} shows the free-field OSMC step response. The step response shows the sequence of the tweeter peak followed by the peaks of of the midrange and the woofer, as expected for a typical 3-way loudspeaker. The transitions between the peaks are smooth.

\figr{osmc_SPL_response} shows the anechoic free-field ($4\pi$) on-axis SPL response. The part of the response curve $>\SI{330}{Hz}$ was determined from the anechoic part of the impulse response. The bass response was determined by converting the $2\pi$ free-field woofer response in \figr{osmc_bass_tuning} to the $4\pi$ system response by applying the baffle-step curve (modelled using Tolvan Edge) and the woofer x-over filter curve (\figr{osmc_filter_transfer}). The the bass \SI{-3}{dB} cut-off is at \SI{50}{Hz}, as targeted in the design goals. Up to \SI{1}{kHz}, the efficiency is \SI{90}{dB-SPL} (\SI{1}{m}, \SI{2.83}{V}). As expected, this is slightly less than the design goal, which is based the theoretically achievable optimum for system zero losses. The small dip at \SI{2.2}{kHz} is a baffle-diffraction effect.\par

*** cumulative decay spectrum

*** SHOW POLAR PLOTS. HORIZONTAL DISPERSION: PROGRESSIVE BEAMING TOWARDS HIGHER FREQUENCIES. THIS REQUIRES TREND OF SLIGHTLY DECREASING ON-AXIS SPL IN ORDER TO ACHIEVE LINEAR POWER RESPONSE (SHOW FIGURE) AND HENCE BALANCED SOUND.


\begin{figure}[tb]
	\centering
	\includegraphics[width=0.7\textwidth]{osmc_step_response}
...
\begin{figure}[tb]
	\centering
	\includegraphics[width=0.7\textwidth]{osmc_SPL_response}
	\caption{Free-field SPL response ($4\pi$), see text.}
