...
\figr{osmc_polar} shows the dispersion of the OSMC. Both the horizontal and vertical dispersion are well controlled are generally very smooth. Beaming increases smoothly towards higher frequencies. The vertical dispersion shows a cancellation at the x-over between the midrange driver and the tweeter. However, due to the steep x-over filters, this artifact is limited to a rather narrow frequency band.

\figr{osmc_powerresponse} shows the OSMC power response, which is the sound power integrated on a sphere around the loudspeaker.\cite{mat_lspr,tylka} The power response reflects the sound power radiated into the listening environment. The slope of power response curve the dashed line is \SI{-1.9}{dB/octave} (dashed line in \figr{osmc_powerresponse}), which conforms with preferred slope determined in controlled listening tests carried out in typical home environments.\cite{osmc_p915}
